...
class Radio extends Input implements InterfaceColumn
{
    ...
    private function fieldOutput(){
        ...
        return '<b-radio v-model="Model.'.$this->field.'" placeholder="請輸入'.$this->name.'" autocomplete="new-'.$this->field.'" true-value="1" false-value="0"></b-radio>';
    }

    private function valuesLoop(){
        $str='<blockquote>';
        foreach($this->values as $key => $value){
            $str.='<div classs="block"><b-radio v-model="Model.'.$this->field.'" native-value="'.$key.'">'.$value.'</b-radio></div>';
        }
        $str.='</blockquote>';
    ...
    private function relationLoop(){
        ...
        if($rows->count() > 0){
            ...
            $rows->map(function($row) use(&$str){
                ...
                $str.='<div classs="block"><b-radio v-model="Model.'.$this->field.'" native-value="'.$row->id.'">'.$col.'</b-radio></div>';
            });
            $str.='</blockquote>';
        }
